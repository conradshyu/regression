...
*/
Regress::Regress(
    const std::list<stREGRESS>& _sample,
...
*/
const std::list<stREGRESS>& Regress::LoadData(
    const std::list<stREGRESS>& _sample,
    unsigned int _degree )
{
    stREGRESS unit; ClearData();

    for ( std::list<stREGRESS>::const_iterator i = _sample.begin(); !( i == _sample.end() ); i++ )
    {
...
*/
const std::list<stREGRESS>& Regress::LoadData(
    const std::vector<double>& _x,  // lambda values
    const std::vector<double>& _y,  // dg/dl; free energy difference
...
{
    std::vector<double> a; a.clear(); a.resize( ( 2 * degree + 1 ), 0.0 );
    std::vector<double> b; b.clear(); b.resize( ( degree + 1 ), 0.0 );

    for ( std::list<stREGRESS>::iterator i = sample.begin(); !( i == sample.end() ); i++ )
    {
...
*/
const std::vector<double>& Regress::GetPolynomial(
    bool _print ) const
{
    if ( _print )
    {
...
double Regress::DoQuadrature(
    bool _print ) const
{
    std::list<stREGRESS>::const_iterator a = sample.begin();
    std::list<stREGRESS>::const_iterator b = sample.begin(); b++;
    double area = 0.0;
